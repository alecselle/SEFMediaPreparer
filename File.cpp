...
bf::path File::path() { return _path; }
...
bf::path File::name() { return _name; }
...
bf::path File::extension() { return _extension; }
...
bool File::loadFileInfo(json j) {
	if (j.find("streams") != j.end()) {
		foreach (auto stream, j.find("streams").value()) {
			...
			if (codec_type.compare("video") == 0 && codec_name.compare("png") != 0 && empty(_vCodec)) {
				_vCodec = codec_name;
			} else if (codec_type.compare("audio") == 0 && empty(_aCodec)) {
				_aCodec = codec_name;
	...
	if (!empty(_vCodec) && !empty(_aCodec) && _duration != NULL) {
...
bool File::isLoaded() { return _loaded; }
