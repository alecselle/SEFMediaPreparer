...
void MediaPreparer::encodeLibrary() {
	...
	for (int i = 0; i < library->getFilesEncode().size() && cancelEncode != true; i++) {
		...
		QProcess process;
		// process.setStandardOutputFile(QString("log.txt"));
		// process.setStandardErrorFile(QString("log.txt"));
...
void MediaPreparer::toggleEncodeFile(int position, int null) {
	...
	if (ui->list_VideoLibrary->item(position, 0)->checkState() == Qt::Checked) {
...
bool MediaPreparer::diffPreset() {
	...
	if (ui->setting_vQuality->value() != stoi(settings->vQuality)) return true;
	if (ui->setting_aQuality->value() != stoi(settings->aQuality)) return true;
	if (ui->setting_container->currentText().toStdString().compare(settings->container) != 0) return true;
	if (ui->setting_subtitles->currentText().toStdString().compare(settings->subtitles) != 0) return true;
	if (ui->setting_threads->text().toStdString().compare(settings->threads) != 0) return true;
	if (ui->setting_extraParams->text().toStdString().compare(settings->extraParams) != 0) return true;
	return false;
}
...
void MediaPreparer::lockUILoad(bool b) {
	...
	if (b || checkConfig()) ui->button_encode->setEnabled(toggle);
...
void MediaPreparer::encodeTab(bool b) {
	if (b) {
		...
		ui->list_encode_VideoLibrary->setRowCount(library->sizeEncode());
		for (int i = 0; i < library->sizeEncode(); i++) {
			File &item = library->getFileEncode(i);
			ui->list_encode_VideoLibrary->setItem(i, 0, new QTableWidgetItem(QString(item.nameStr().c_str())));
			ui->list_encode_VideoLibrary->setItem(i, 1, new QTableWidgetItem(QString(item.vcodec().c_str())));
			ui->list_encode_VideoLibrary->setItem(i, 2, new QTableWidgetItem(QString(item.acodec().c_str())));
			ui->list_encode_VideoLibrary->setItem(i, 3, new QTableWidgetItem(QString(item.extensionStr().c_str())));
			ui->list_encode_VideoLibrary->setItem(i, 4, new QTableWidgetItem(QString(item.subtitlesStr().c_str())));
		}
...
void MediaPreparer::Init() {
	...
	// connect(ui->list_VideoLibrary, SIGNAL(cellChanged(int, int)), this, SLOT(toggleEncodeFile(int, int)));
	...
	if (!settings->preserveLog) bf::remove(settings->baseDir.string() + "\\log.txt");
...
void MediaPreparer::InitGUI() {
	...
	/*
	ui->list_VideoLibrary->setColumnWidth(0, 20);
	ui->list_VideoLibrary->setColumnWidth(1, 155);
	ui->list_VideoLibrary->setColumnWidth(2, 65);
	ui->list_VideoLibrary->setColumnWidth(3, 65);
	ui->list_VideoLibrary->setColumnWidth(4, 65);
	ui->list_VideoLibrary->setColumnWidth(5, 65);
	*/

	ui->list_VideoLibrary->setColumnWidth(0, 155);
	ui->list_VideoLibrary->setColumnWidth(1, 70);
	ui->list_VideoLibrary->setColumnWidth(2, 70);
	ui->list_VideoLibrary->setColumnWidth(3, 70);
	ui->list_VideoLibrary->setColumnWidth(4, 70);
...
void MediaPreparer::log(QString msg) { log(msg.toStdString()); }
...
void MediaPreparer::workerScanAddItem(int pos) {
	...
	ui->list_VideoLibrary->setRowCount(pos + 1);
	// ui->list_VideoLibrary->setItem(pos, 0, new QTableWidgetItem());
	// ui->list_VideoLibrary->item(pos, 0)->setCheckState(Qt::Checked);
	ui->list_VideoLibrary->setItem(pos, 0, new QTableWidgetItem(QString(item.nameStr().c_str())));
	ui->list_VideoLibrary->setItem(pos, 1, new QTableWidgetItem(QString(item.vcodec().c_str())));
	ui->list_VideoLibrary->setItem(pos, 2, new QTableWidgetItem(QString(item.acodec().c_str())));
	ui->list_VideoLibrary->setItem(pos, 3, new QTableWidgetItem(QString(item.extensionStr().c_str())));
	ui->list_VideoLibrary->setItem(pos, 4, new QTableWidgetItem(QString(item.subtitlesStr().c_str())));
	blockAllSignals(false);
}
...
void MediaPreparer::workerEncodeChangeItem(int pos) {
	...
	ui->list_encode_VideoLibrary->removeRow(0);
