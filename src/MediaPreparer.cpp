...
namespace SuperEpicFuntime {
	...
	void MediaPreparer::blockAllSignals(bool b) {
		...
		foreach (QWidget *x, widgets) // @suppress("Invalid arguments") // @suppress("Field cannot be resolved")
		{
			x->blockSignals(b);
	...
	void MediaPreparer::workerScanAddItem(int pos) {
		...
		ui->label_fileCount->setText(QString(("<html><head/><body><p>" + std::to_string(pos + 1) + " file(s) found</p></body></html>").c_str())); // @suppress("Function cannot be resolved") // @suppress("Method cannot be resolved")
