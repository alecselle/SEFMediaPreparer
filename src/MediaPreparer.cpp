    ...
    if (workerThread.isRunning() && workerType == ENCODE) {
        ...
        if (--updateOffset <= 0) {
            ...
            if (boost::filesystem::exists(settings->tempDir + "\\" + workerItem.name() + ".txt")) {
                std::ifstream fileHandler(settings->tempDir + "\\" + workerItem.name() + ".txt", std::ifstream::in);
...
void MediaPreparer::runWorker_scan() {
    if (!workerThread.isRunning()) {
        loadSettings_gui();
        worker = Worker(SCAN);
        //workerThread = QtConcurrent::run(&Worker::run, worker); //qt6
        workerThread = QtConcurrent::run(worker, &Worker::run); //qt5
    }
}
...
void MediaPreparer::eventListener(Event *e) {
	...
	.arg(((workerItemTimeStamp.elapsed() % 3600000) % 60000) / 1000, 2, 10, QChar('0')));
            ...
            ui->list_complete_Library->setItem(eventData, 1,
...
void MediaPreparer::blockSignals(bool b) {
    foreach (QWidget *x, ui->container_main->findChildren<QWidget *>()) {
        x->blockSignals(b);
    }
}
