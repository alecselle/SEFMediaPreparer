...
#define ENCODE_SECTION {
	...
	params.push_back(p);
        ...
        params += {"-metadata", ("title=" + f.name()).c_str(), "-metadata", "comment=Processed by SuperEpicFuntime Media Preparer", "-strict", "-2",
        ...
        if (!cancelWorker) {
            ...
            if (settings->subfolders) {
                ...
                boost::filesystem::rename(settings->tempDir + "\\" + f.name() + "." + settings->container,
	settings->outputDir + "\\" + f.name() + "\\" + f.name() + "." + settings->container);
            } else {
...
#define END_ENCODE_SECTION }
    ...
    void run() {
        ...
        if (eventHandler != NULL && settings != NULL && library != NULL) {
            switch (type) {
            case SCAN: worker_scan(); break;
            case ENCODE: worker_encode(); break;
            default: break;
            }
        }
    }
