...
void Settings::savePreset() { savePresetAs(presetName); }
...
void Settings::loadConfig() {
	FILE *fp = fopen(CONFIG_FILE.string().c_str(), "r");
	auto j = json::parse(ifstream(fp));
...
void Settings::loadPreset() { loadPresetFile(presetPath); }

void Settings::loadPreset(string name) { loadPresetFile(PRESET_DIR.string() + "\\" + name + ".preset"); }
...
void Settings::loadPresetFile(bf::path path) {
	if (bf::exists(path)) {
		...
		FILE *fp = fopen(DEFAULT_PRESET.string().c_str(), "r");
		json j = json::parse(ifstream(fp));
		presetPath = path;
		presetName = path.filename().replace_extension().string();
		loadPresetJson(j);
